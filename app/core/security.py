...
_encode_token = partial(
    jwt.encode, key=settings.SECRET_KEY, algorithm=settings.ALGORITHM
)
_decode_token = partial(
...
def verify_password(plain_password: str, hashed_password: str) -> bool:
    return compose(_sha256_hex, partial(_bcrypt_verify, hashed=hashed_password))(
        plain_password
    )
