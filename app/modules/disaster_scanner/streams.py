        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    "https://eonet.gsfc.nasa.gov/api/v3/events", headers=headers
                ) as resp:
                    if resp.status == 200:
                        data = await resp.json(content_type=None)
                        events = data.get("events", [])
                        print(
                            f">>> [NASA] SUCCESS! Found {len(events)} events.",
                            flush=True,
                        )
                        for event in events:
                            yield event
                        ...
                        print(
                            f">>> [NASA] API returned status {resp.status}", flush=True
                        )
                ...
                async with session.get(
                    ...
                    if resp.status == 200:
                        ...
                        if isinstance(data, list):
                            print(
                                f">>> [PDC] SUCCESS! Found {len(data)} events.",
                                flush=True,
                            )
                            for event in data:
